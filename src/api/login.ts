...
export const login = async (args: LoginRequest): Promise<LoginResult> => {
  ...
  const isLogin = await loginWithToken(args);
  if (isLogin.result === 'fail') return 'fail';

  saveAccessTokenToLocalStorage(isLogin.access_token);
  return 'success';
};
...
export const getCurrentUserInfo = async (): Promise<UserInfo | null> => {
  ...
  const userInfo = await getCurrentUserInfoWithToken(token);
